...
def do_quantity_plot(ds: Dataset, xcol: Column, ycols: list[Column]) -> plt.Figure:
    ...
    for idx, did in enumerate(sorted(ds.keys())):
        ...
        for ycol in ycols:
            ...
            df[[xcol, ycol]].mean().plot(
                x=xcol, y=ycol, kind="line", ax=ax, label=did, **get_marker(idx)
            )
            low, high = df[ycol].min(), df[ycol].max()
            ax.fill_between(list(sorted(df.groups)), low, high, alpha=0.2, color=palette.pop(0))

    set_up_figure(f, names=(xcol.idx, ycols[0].idx))
    if ycols[0].formatter:
        ax.yaxis.set_major_formatter(ycols[0].formatter)
    return f


def do_quantity_log_plot(
