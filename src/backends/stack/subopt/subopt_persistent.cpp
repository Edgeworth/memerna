...
int64_t SuboptPersistent<UseLru>::Run(const SuboptCallback& fn) {
  ...
  const auto start_idx = base::DpIndex(0, INVALID_INDEX, EXT);
...
SuboptPersistent<UseLru>::RunInternal() {
  while (!pq_.empty()) {
    ...
    if (!exp.idx0.has_value() && s.unexpanded_idx != INVALID_SUBOPT_INDEX) {
...
template <bool UseLru>
std::vector<Expansion> SuboptPersistent<UseLru>::ExtExpansions(
  ...
  for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
    ...
    if (cfg_.UseDangleMismatch()) {
      ...
      // 5(   )<   > 5'
      energy = base10 + m_->dangle5[enb][stb][st1b] + pf_.Unpaired(st) + ext[en + 1][EXT];
      if (energy <= delta)
        exps.push_back({.delta = energy,
...
std::vector<Expansion> SuboptPersistent<UseLru>::PairedOrNoStackExpansions(
  ...
  energy = m_->Hairpin(r_, pf_, st, en) - target;
  if (energy <= delta) exps.push_back({.delta = energy, .pair{st, en}});

  const auto base_branch_cost =
      m_->AuGuPenalty(stb, enb) + pf_.Paired(st, en) + m_->multiloop_a + m_->multiloop_b - target;
  // (<   ><    >)
  ...
  if (cfg_.UseDangleMismatch()) {
    ...
    energy = base_branch_cost + dp[st + 2][en - 2][DP_U2] + m_->terminal[stb][st1b][en1b][enb] +
        pf_.Unpaired(st + 1) + pf_.Unpaired(en - 1) + 2 * m_->multiloop_c;
    if (energy <= delta)
      exps.push_back({.delta = energy,
  ...
  if (cfg_.UseCoaxialStacking()) {
    ...
    for (int piv = st + HAIRPIN_MIN_SZ + 2; piv < en - HAIRPIN_MIN_SZ - 2; ++piv) {
      ...
      energy = base_branch_cost + dp[st + 2][piv - 1][DP_P] + m_->multiloop_b +
          ...
          m_->MismatchCoaxial(pl1b, plb, st1b, st2b) + pf_.Unpaired(st + 1) + pf_.Unpaired(piv) +
          2 * m_->multiloop_c;
      if (energy <= delta)
        exps.push_back({.delta = energy,
      ...
      energy = base_branch_cost + dp[st + 1][piv][DP_U] + m_->multiloop_b +
          ...
          m_->MismatchCoaxial(en2b, en1b, prb, pr1b) + pf_.Unpaired(piv + 1) +
          pf_.Unpaired(en - 1) + 2 * m_->multiloop_c;
      if (energy <= delta)
        exps.push_back({.delta = energy,
...
std::vector<Expansion> SuboptPersistent<UseLru>::UnpairedExpansions(
  ...
  if (st + 1 < en && (a == DP_U || a == DP_U2)) {
    energy = dp[st + 1][en][a] + pf_.Unpaired(st) + m_->multiloop_c - dp[st][en][a];
