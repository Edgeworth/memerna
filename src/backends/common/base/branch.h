...
#ifndef BACKENDS_COMMON_BASE_BRANCH_H_
#define BACKENDS_COMMON_BASE_BRANCH_H_
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (ru_usable[i]) {
      if (m.cfg().UseDangleMismatch()) {
        ...
        UPDATE_CACHE(ru_shared[i], i + 1, 1, i, m.dangle3[rb][rub][lb], CTD_3_DANGLE);
      }
      ...
      if (m.cfg().UseCoaxialStacking() && ru_shared[i] && i != N - 1 && ru_usable[i + 1]) {
        ...
        if (lu_exists[i]) {
          // In the case that lu doesn't exist but it is "used" it means this branch was consumed
          // by a coaxial interaction so don't use it.
...
Energy AddBaseCtdsToBranchCtds(const T& m, const Primary& r, const Secondary& s, const Ctds& ctd,
  ...
  for (int i = 0; i < static_cast<int>(branches.size()); ++i) {
    ...
    case CTD_FCOAX_WITH_PREV:
      // (   )(   ) or ((   )   ) or (   (   ))
      energy = m.stack[r[s[prev_branch]]][stb][enb][r[prev_branch]];
...
#endif  // BACKENDS_COMMON_BASE_BRANCH_H_
