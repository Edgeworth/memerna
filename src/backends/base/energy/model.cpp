...
Energy Model::MultiloopEnergy(const Primary& r, const erg::PseudofreeCfg& pf, const Secondary& s,
...
Energy Model::SubEnergyInternal(const Primary& r, const Secondary& s, const erg::PseudofreeCfg& pf,
  ...
  if (exterior_loop || branches.size() >= 2) {
    // Multiloop.
    energy += MultiloopEnergy(r, pf, s, st, en, &branches, use_given_ctds, ctd, struc);
  ...
  for (auto i : branches) {
    if (struc) {
      ...
      energy += SubEnergyInternal(r, s, pf, i, s[i], use_given_ctds, ctd, &sstruc);
      (*struc)->AddBranch(std::move(sstruc));
    } else {
      energy += SubEnergyInternal(r, s, pf, i, s[i], use_given_ctds, ctd, nullptr);
    }
  }
...
EnergyResult Model::SubEnergy(const Primary& r, const Secondary& s, const Ctds* given_ctd,
  ...
  auto energy =
      SubEnergyInternal(r, s, pf, st, en, use_given_ctds, &ctd, build_structure ? &struc : nullptr);
