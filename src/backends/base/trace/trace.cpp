  ...
  while (!q.empty()) {
    ...
    if (en == -1) {
      ...
      if (a == DP_P) {
        ...
        if (m->Hairpin(r, pf, st, en) == dp[st][en][DP_P]) {
          goto loopend;
        }
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        if (a != DP_U2 &&
            pf.UnpairedSum(piv + 1, en) + (en - piv) * m->multiloop_c < right_unpaired) {
          right_unpaired = pf.UnpairedSum(piv + 1, en) + (en - piv) * m->multiloop_c;
        ...
        if (cfg.UseDangleMismatch()) {
          ...
          // 5(   )<   > 5' - U, U2
          if (base10 + m->dangle5[pb][stb][st1b] + pf.Unpaired(st) + m->multiloop_c +
                  right_unpaired ==
              dp[st][en][a]) {
            res.ctd[st + 1] = CTD_5_DANGLE;
            q.emplace_back(st + 1, piv, DP_P);
        ...
        if (cfg.UseCoaxialStacking()) {
          ...
          // (   )(<   ) > Flush coax - U, U2
          if (base01 + m->stack[pl1b][pb][WcPair(pb)][stb] + dp[piv][en][DP_U_WC] ==
              dp[st][en][a]) {
            res.ctd[st] = CTD_FCOAX_WITH_NEXT;
          ...
          if ((IsGu(pb)) &&
              base01 + m->stack[pl1b][pb][GuPair(pb)][stb] + dp[piv][en][DP_U_GU] ==
                  dp[st][en][a]) {
            res.ctd[st] = CTD_FCOAX_WITH_NEXT;
            res.ctd[piv] = CTD_FCOAX_WITH_PREV;
            q.emplace_back(st, piv - 1, DP_P);
