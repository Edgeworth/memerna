...
int64_t SuboptDebug::Run(const SuboptCallback& fn) {
  ...
  while (!q_.empty()) {
    ...
    if (a == DP_P) {
      ...
      // Hairpin loop
      energy = base_energy + m_->Hairpin(r_, pf_, st, en);
      Expand(energy);

      auto base_and_branch = base_energy + m_->AuGuPenalty(stb, enb) + pf_.Paired(st, en) +
      ...
      if (cfg_.UseCoaxialStacking()) {
        ...
        for (int piv = st + HAIRPIN_MIN_SZ + 2; piv < en - HAIRPIN_MIN_SZ - 2; ++piv) {
          ...
          energy = base_and_branch + dp_.dp[st + 2][piv - 1][DP_P] + m_->multiloop_b +
              ...
              m_->MismatchCoaxial(pl1b, plb, st1b, st2b) + pf_.Unpaired(st + 1) +
      ...
      if (st + 1 < en && (a == DP_U || a == DP_U2)) {
        energy = base_energy + pf_.Unpaired(st) + m_->multiloop_c + dp_.dp[st + 1][en][a];
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        if (cfg_.UseDangleMismatch()) {
          ...
          // 5(   )<   > 5' - U, U2
          energy = base_energy + base10 + m_->dangle5[pb][stb][st1b] + pf_.Unpaired(st) +
              m_->multiloop_c;
          if (a == DP_U)
          ...
          // .(   ).<   > Terminal mismatch - U, U2
          energy = base_energy + base11 + m_->terminal[pl1b][pb][stb][st1b] + pf_.Unpaired(st) +
              pf_.Unpaired(piv) + 2 * m_->multiloop_c;
          if (a == DP_U)
