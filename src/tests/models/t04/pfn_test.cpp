...
TEST_P(PfnTestT04, T04P1) {
  auto [i, alg] = GetParam();
  auto m = t04_ms[i];
  if (!PfnAlgIsSupported(GetBackendKind(m), alg, erg::EnergyCfg{}, erg::PseudofreeCfg{}, /*reason=*/nullptr))
    return;
...
TEST(PfnTestT04, PseudofreeMatchesBruteAndScalesQ) {
  // BRUTE is expensive; keep this very small.
  ...
  EXPECT_REL_EQ(brute.q, no_pf.q * scale);
}
