...
void InitTest(const std::string& data_dir) {
  ...
  while (base_ms.size() < NUM_TEST_MODELS)
    base_ms.push_back(md::base::Model::Random(kT04Cfg,
        RandomModelCfg(static_cast<uint_fast32_t>(base_ms.size()), E(-10.0), E(10.0))));
  ...
  while (baseopt_ms.size() < NUM_TEST_MODELS)
    baseopt_ms.push_back(md::base::opt::Model::Random(kT04Cfg,
        RandomModelCfg(static_cast<uint_fast32_t>(baseopt_ms.size()), E(-10.0), E(10.0))));
  ...
  while (stack_ms.size() < NUM_TEST_MODELS)
    stack_ms.push_back(md::stack::Model::Random(kT04Cfg,
        RandomModelCfg(static_cast<uint_fast32_t>(stack_ms.size()), E(-10.0), E(10.0))));
}
