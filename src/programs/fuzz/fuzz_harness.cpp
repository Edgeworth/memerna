...
FuzzHarness::FuzzHarness(mrna::ArgParse args)
    : args_(std::move(args)), em_(mrna::energy::FromArgParse(args_)),
...
mrna::fuzz::FuzzInvocation FuzzHarness::CreateInvocation(const mrna::Primary& r) {
  ...
  if (args_.GetOr(OPT_RANDOM_MODEL))
    invoc = mrna::fuzz::FuzzInvocation(
        r, mrna::energy::Random(args_.Get<ModelKind>(OPT_MEMERNA_MODEL), e_()), cfg_);
#ifdef USE_RNASTRUCTURE
