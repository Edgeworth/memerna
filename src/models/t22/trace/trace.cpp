...
TraceResult Traceback(const Primary& r, const Model::Ptr& em, const DpState& state) {
  ...
  const auto& [t04state, penult] = state;
  ...
  while (!q.empty()) {
    ...
    loopend0 : {};
      ...
      const auto stb = r[st];
      const auto enb = r[en];
      const auto augu_penalty = em->AuGuPenalty(stb, enb);
      ...
      if (length > 1) {
        ...
        if (none == penult[st][en][length]) {
          q.emplace(PenultimateIndex(st + 1, en - 1, length - 1));
          goto loopend1;
        }
        auto left = penult[st + 2][en - 1][length - 1] + bulge_left;
        if (left == penult[st][en][length]) {
          q.emplace(PenultimateIndex(st + 2, en - 1, length - 1));
          goto loopend1;
        }
        auto right = penult[st + 1][en - 2][length - 1] + bulge_right;
        // fmt::print("{} {} {} {} {}\n", st, en, bulge_left, bulge_right, right);
        if (right == penult[st][en][length]) {
          q.emplace(PenultimateIndex(st + 1, en - 2, length - 1));
          goto loopend1;
        }
      } else {
        ...
        const auto end = em->penultimate_stack[r[st - 1]][stb][enb][r[en + 1]];
        if (dp[st + 1][en - 1][DP_U2] + augu_penalty + end == penult[st][en][length]) {
          q.emplace(t04::Index(st + 1, en - 1, DP_U2));
          goto loopend1;
        }

        if (em->Hairpin(r, st, en) + end == penult[st][en][length]) {
          // fmt::print("hairpin: {} {}\n", em->Hairpin(r, st, en), en);
          goto loopend1;
        }
        ...
        for (int ist = st + 1; ist < st + max_inter + 2; ++ist) {
          for (int ien = en - max_inter + ist - st - 2; ien < en; ++ien) {
            // Try all internal loops. We don't check stacks or 1 nuc bulge loops.
            if (dp[ist][ien][DP_P] < CAP_E && ist - st + en - ien > 3) {
              const auto val = em->TwoLoop(r, st, en, ist, ien) + dp[ist][ien][DP_P] + end;
              // fmt::print("twoloop: {} {} {} {} {}\n", ist, ien, em->TwoLoop(r, st, en, ist, ien),
              // val, dp[st][en][DP_P]);
              if (val == penult[st][en][length]) {
                q.emplace(t04::Index(ist, ien, DP_P));
                goto loopend1;
              }
            }
