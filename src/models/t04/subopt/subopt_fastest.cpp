  ...
  if (en == -1) {
    if (a == EXT) {
      ...
      if (st == N)
        ...
        exps.push_back({dp_.ext[st + 1][EXT] - dp_.ext[st][a], {st + 1, -1, EXT}});
    }
    for (en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      if (en < N - 1) {
        ...
        if (energy + dp_.ext[en + 1][EXT_GU] <= delta)
          exps.push_back(
              {energy + dp_.ext[en + 1][EXT_GU], {en + 1, -1, EXT_GU}, {st + 1, en - 1, DP_P},
                  {en + 1, CTD_LCOAX_WITH_PREV}, {st + 1, CTD_LCOAX_WITH_NEXT}});
        if (energy + dp_.ext[en + 1][EXT_WC] <= delta)
          exps.push_back(
              {energy + dp_.ext[en + 1][EXT_WC], {en + 1, -1, EXT_WC}, {st + 1, en - 1, DP_P},
                  {en + 1, CTD_LCOAX_WITH_PREV}, {st + 1, CTD_LCOAX_WITH_NEXT}});
      }
  ...
  for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
    ...
    if (a == DP_U_RC) {
      ...
      if (energy + dp_.dp[piv + 1][en][DP_U] <= delta)
        exps.push_back(
            {energy + dp_.dp[piv + 1][en][DP_U], {st + 1, piv - 1, DP_P}, {piv + 1, en, DP_U}});
      continue;
    }
    ...
    if (a == DP_U) {
      ...
      if (energy + dp_.dp[piv + 1][en][DP_U] <= delta)
        exps.push_back({energy + dp_.dp[piv + 1][en][DP_U], {st, piv, DP_P}, {piv + 1, en, DP_U},
            {st, CTD_UNUSED}});
    }
    ...
    if (a == DP_U_WC || a == DP_U_GU) {
      ...
      if ((a == DP_U_WC && IsWcPair(stb, pb)) || (a == DP_U_GU && IsGuPair(stb, pb))) {
        ...
        if (energy + dp_.dp[piv + 1][en][DP_U] <= delta)
          exps.push_back(
              {energy + dp_.dp[piv + 1][en][DP_U], {st, piv, DP_P}, {piv + 1, en, DP_U}});
      }
      continue;
    }
    ...
    if (energy + dp_.dp[piv + 1][en][DP_U] <= delta)
      exps.push_back({energy + dp_.dp[piv + 1][en][DP_U], {st, piv - 1, DP_P}, {piv + 1, en, DP_U},
          {st, CTD_3_DANGLE}});
    ...
    if (energy + dp_.dp[piv + 1][en][DP_U] <= delta)
      exps.push_back({energy + dp_.dp[piv + 1][en][DP_U], {st + 1, piv, DP_P}, {piv + 1, en, DP_U},
          {st + 1, CTD_5_DANGLE}});
    ...
    if (energy + dp_.dp[piv + 1][en][DP_U] <= delta)
      exps.push_back({energy + dp_.dp[piv + 1][en][DP_U], {st + 1, piv - 1, DP_P},
          {piv + 1, en, DP_U}, {st + 1, CTD_MISMATCH}});

    // .(   ).<(   ) > Left coax - U, U2
    energy = base11 + em_->MismatchCoaxial(pl1b, pb, stb, st1b);
    if (energy + dp_.dp[piv + 1][en][DP_U_WC] <= delta)
      exps.push_back({energy + dp_.dp[piv + 1][en][DP_U_WC], {st + 1, piv - 1, DP_P},
          {piv + 1, en, DP_U_WC}, {st + 1, CTD_LCOAX_WITH_NEXT}, {piv + 1, CTD_LCOAX_WITH_PREV}});
    if (energy + dp_.dp[piv + 1][en][DP_U_GU] <= delta)
      exps.push_back({energy + dp_.dp[piv + 1][en][DP_U_GU], {st + 1, piv - 1, DP_P},
