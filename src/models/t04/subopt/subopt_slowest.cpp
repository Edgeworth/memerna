  ...
  while (!q_.empty()) {
    ...
    if (en == -1) {
      ...
      for (en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
        ...
        const auto base00 = dp_.dp[st][en][DP_P] + em_->AuGuPenalty(stb, enb);
        const auto base01 = dp_.dp[st][en - 1][DP_P] + em_->AuGuPenalty(stb, en1b);
        const auto base10 = dp_.dp[st + 1][en][DP_P] + em_->AuGuPenalty(st1b, enb);
        const auto base11 = dp_.dp[st + 1][en - 1][DP_P] + em_->AuGuPenalty(st1b, en1b);
        curnode_ = node.copy();
        ...
        // (   )3<   > 3'
        energy = base_energy + base01 + em_->dangle3[en1b][enb][stb] + dp_.ext[en + 1][EXT];
        Expand(energy, {en + 1, -1, EXT}, {st, en - 1, DP_P}, {st, CTD_3_DANGLE});

        // 5(   )<   > 5'
        energy = base_energy + base10 + em_->dangle5[enb][stb][st1b] + dp_.ext[en + 1][EXT];
        Expand(energy, {en + 1, -1, EXT}, {st + 1, en, DP_P}, {st + 1, CTD_5_DANGLE});

        // .(   ).<   > Terminal mismatch
        energy = base_energy + base11 + em_->terminal[en1b][enb][stb][st1b] + dp_.ext[en + 1][EXT];
        Expand(energy, {en + 1, -1, EXT}, {st + 1, en - 1, DP_P}, {st + 1, CTD_MISMATCH});

        // (   )(<   ) > Flush coax
        energy =
            base_energy + base01 + em_->stack[en1b][enb][WcPair(enb)][stb] + dp_.ext[en][EXT_WC];
        Expand(energy, {en, -1, EXT_WC}, {st, en - 1, DP_P}, {en, CTD_FCOAX_WITH_PREV},
            {st, CTD_FCOAX_WITH_NEXT});
        if (IsGu(enb)) {
          energy =
              base_energy + base01 + em_->stack[en1b][enb][GuPair(enb)][stb] + dp_.ext[en][EXT_GU];
    ...
    if (a == DP_P) {
      ...
      for (int ist = st + 1; ist < st + max_inter + 2; ++ist) {
        for (int ien = en - max_inter + ist - st - 2; ien < en; ++ien) {
          energy = base_energy + em_->TwoLoop(r_, st, en, ist, ien) + dp_.dp[ist][ien][DP_P];
      ...
      // (.<   ><   >.) Terminal mismatch
      energy =
          base_and_branch + dp_.dp[st + 2][en - 2][DP_U2] + em_->terminal[stb][st1b][en1b][enb];
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 2; piv < en - HAIRPIN_MIN_SZ - 2; ++piv) {
        ...
        energy = base_and_branch + dp_.dp[st + 2][piv][DP_P] + em_->multiloop_hack_b +
            em_->AuGuPenalty(st2b, plb) + dp_.dp[piv + 1][en - 2][DP_U] + outer_coax;
        Expand(energy, {st + 2, piv, DP_P}, {piv + 1, en - 2, DP_U}, {st + 2, CTD_LCOAX_WITH_PREV},
            {en, CTD_LCOAX_WITH_NEXT});

        // (.   (   ).) Right outer coax
        energy = base_and_branch + dp_.dp[st + 2][piv][DP_U] + em_->multiloop_hack_b +
            em_->AuGuPenalty(prb, en2b) + dp_.dp[piv + 1][en - 2][DP_P] + outer_coax;
        Expand(energy, {st + 2, piv, DP_U}, {piv + 1, en - 2, DP_P}, {piv + 1, CTD_RC_WITH_NEXT},
            {en, CTD_RC_WITH_PREV});

        // (.(   ).   ) Left inner coax
        energy = base_and_branch + dp_.dp[st + 2][piv - 1][DP_P] + em_->multiloop_hack_b +
            em_->AuGuPenalty(st2b, pl1b) + dp_.dp[piv + 1][en - 1][DP_U] +
            em_->MismatchCoaxial(pl1b, plb, st1b, st2b);
        Expand(energy, {st + 2, piv - 1, DP_P}, {piv + 1, en - 1, DP_U}, {st + 2, CTD_RC_WITH_PREV},
            {en, CTD_RC_WITH_NEXT});

        // (   .(   ).) Right inner coax
        energy = base_and_branch + dp_.dp[st + 1][piv][DP_U] + em_->multiloop_hack_b +
            em_->AuGuPenalty(pr1b, en2b) + dp_.dp[piv + 2][en - 2][DP_P] +
        ...
        // ((   )   ) Left flush coax
        energy = base_and_branch + dp_.dp[st + 1][piv][DP_P] + em_->multiloop_hack_b +
            em_->AuGuPenalty(st1b, plb) + dp_.dp[piv + 1][en - 1][DP_U] +
        ...
        // (   (   )) Right flush coax
        energy = base_and_branch + dp_.dp[st + 1][piv][DP_U] + em_->multiloop_hack_b +
            em_->AuGuPenalty(prb, en1b) + dp_.dp[piv + 1][en - 1][DP_P] +
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        // right.
        auto base00 = dp_.dp[st][piv][DP_P] + em_->AuGuPenalty(stb, pb) + em_->multiloop_hack_b;
        auto base01 =
            dp_.dp[st][piv - 1][DP_P] + em_->AuGuPenalty(stb, pl1b) + em_->multiloop_hack_b;
        auto base10 =
            dp_.dp[st + 1][piv][DP_P] + em_->AuGuPenalty(st1b, pb) + em_->multiloop_hack_b;
        auto base11 =
            dp_.dp[st + 1][piv - 1][DP_P] + em_->AuGuPenalty(st1b, pl1b) + em_->multiloop_hack_b;
        ...
        // (   )(<   ) > Flush coax - U, U2
        energy =
            base_energy + base01 + em_->stack[pl1b][pb][WcPair(pb)][stb] + dp_.dp[piv][en][DP_U_WC];
        Expand(energy, {st, piv - 1, DP_P}, {piv, en, DP_U_WC}, {st, CTD_FCOAX_WITH_NEXT},
            {piv, CTD_FCOAX_WITH_PREV});

        if (IsGu(pb)) {
          energy = base_energy + base01 + em_->stack[pl1b][pb][GuPair(pb)][stb] +
