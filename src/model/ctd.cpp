...
Ctds ParseD2String(const Secondary& s, const std::string& ctd_str) {
  const auto N = As<int>(ctd_str.size());
...
Ctds ParseCtdString(const Secondary& s, const std::string& ctd_str) {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (c == 'P' || c == 'p') {
      ...
      if (s.IsUnpaired(prev)) {
        --prev;
        mismatch = true;
      }
      verify(prev >= 0 && s.IsPaired(prev) && (ctd_str[s[prev]] == 'n' || ctd_str[s[prev]] == 'N'),
          "invalid input");
    ...
    if (c == '3') {
      ...
      // here.
      verify(i - 1 >= 0 && s.IsPaired(i - 1) &&
              (ctd[s[i - 1]] == CTD_NA || ctd[s[i - 1]] == CTD_UNUSED),
          "invalid input");
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (c == 'm') {
      const bool right_branch_viable =
          i + 1 < N && s.IsPaired(i + 1) && s[i + 1] + 1 < N && ctd_str[s[i + 1] + 1] == 'M';
...
std::string Ctds::ToString(const Secondary& s, bool use_d2) const {
  ...
  if (use_d2) {
    ...
    for (int i = 0; i < N; ++i) {
      if (s.IsUnpaired(i)) continue;
      const bool closing = s.IsClosingPair(i);
      if (closing)
        str[i] = ']';
      else
        str[i] = '[';
  ...
  for (int i = 0; i < N; ++i) {
    ...
    case CTD_5_DANGLE:
      ...
      break;
    case CTD_MISMATCH:
      assert(i > 0 && s.IsPaired(i) && s[i] + 1 < N);
      str[i - 1] = 'm';
      str[s[i] + 1] = 'M';
      break;
