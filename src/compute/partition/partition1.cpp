  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      if (energy::ViableFoldingPair(st, en)) {
        ...
        const PEnergy base_branch_cost = gppc.augubranch[stb][enb] * gppc.em.multiloop_hack_a;
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        const PEnergy base00 = gpt[st][piv][PT_P] * gppc.augubranch[stb][pb];
        const PEnergy base01 = gpt[st][piv - 1][PT_P] * gppc.augubranch[stb][pl1b];
        const PEnergy base10 = gpt[st + 1][piv][PT_P] * gppc.augubranch[st1b][pb];
        const PEnergy base11 = gpt[st + 1][piv - 1][PT_P] * gppc.augubranch[st1b][pl1b];
  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = 0; en < st; ++en) {
      ...
      if (energy::ViableFoldingPair(en, st)) {
        ...
        {
          const PEnergy augu = gppc.em.AuGuPenalty(enb, stb);
      ...
      for (int tpiv = st + 1; tpiv <= en + N; ++tpiv) {
        ...
        const PEnergy base00 = gpt[st][piv][PT_P] * gppc.augubranch[stb][pb];
        const PEnergy base01 = gpt[st][pl][PT_P] * gppc.augubranch[stb][pl1b];
        ...
        if (lspace) {
          const PEnergy base10 = gpt[st + 1][piv][PT_P] * gppc.augubranch[st1b][pb];
          const PEnergy base11 = gpt[st + 1][pl][PT_P] * gppc.augubranch[st1b][pl1b];
