  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        const BoltzEnergy base00 = dp[st][piv][PT_P] * Boltz(pc.augubranch[stb][pb]);
        const BoltzEnergy base01 = dp[st][piv - 1][PT_P] * Boltz(pc.augubranch[stb][pl1b]);
        const BoltzEnergy base10 = dp[st + 1][piv][PT_P] * Boltz(pc.augubranch[st1b][pb]);
        const BoltzEnergy base11 = dp[st + 1][piv - 1][PT_P] * Boltz(pc.augubranch[st1b][pl1b]);
  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = 0; en < st; ++en) {
      ...
      for (int tpiv = st + 1; tpiv <= en + N; ++tpiv) {
        ...
        // baseAB indicates A bases left unpaired on the left, B bases left unpaired on the right.
        const BoltzEnergy base00 = dp[st][piv][PT_P] * Boltz(pc.augubranch[stb][pb]);
        const BoltzEnergy base01 = dp[st][pl][PT_P] * Boltz(pc.augubranch[stb][pl1b]);
        ...
        if (lspace) {
          const BoltzEnergy base10 = dp[st + 1][piv][PT_P] * Boltz(pc.augubranch[st1b][pb]);
          const BoltzEnergy base11 = dp[st + 1][pl][PT_P] * Boltz(pc.augubranch[st1b][pl1b]);
