  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      if (ViableFoldingPair(r, st, en)) {
        ...
        // (3<   ><   >) 3'
        p += base_branch_cost * gpt[st + 2][en - 1][PT_U2] * Boltz(em.dangle3[stb][st1b][enb]);
        // (<   ><   >5) 5'
        p += base_branch_cost * gpt[st + 1][en - 2][PT_U2] * Boltz(em.dangle5[stb][en1b][enb]);
        // (.<   ><   >.) Terminal mismatch
        ...
        for (int piv = st + HAIRPIN_MIN_SZ + 2; piv < en - HAIRPIN_MIN_SZ - 2; ++piv) {
          ...
          p += base_branch_cost * gpt[st + 2][piv][PT_P] * gpt[piv + 1][en - 2][PT_U] *
              Boltz(pc.augubranch[st2b][plb] + outer_coax);
          // (.   (   ).) Right outer coax
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        const BoltzEnergy base00 = gpt[st][piv][PT_P] * Boltz(pc.augubranch[stb][pb]);
        const BoltzEnergy base01 = gpt[st][piv - 1][PT_P] * Boltz(pc.augubranch[stb][pl1b]);
        const BoltzEnergy base10 = gpt[st + 1][piv][PT_P] * Boltz(pc.augubranch[st1b][pb]);
        const BoltzEnergy base11 = gpt[st + 1][piv - 1][PT_P] * Boltz(pc.augubranch[st1b][pl1b]);
        ...
        // (   )(<   ) > Flush coax - U
        val = base01 * Boltz(em.stack[pl1b][pb][pb ^ 3][stb]) * gpt[piv][en][PT_U_WC];
        u += val;
        u2 += val;
        if (pb == G || pb == U) {
          val = base01 * Boltz(em.stack[pl1b][pb][pb ^ 1][stb]) * gpt[piv][en][PT_U_GU];
          u += val;
          u2 += val;
        }
      }
  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = 0; en < st; ++en) {
      ...
      if (ViableFoldingPair(r, en, st)) {
        ...
        // Try being an exterior loop - coax cases handled in the loop after this.
        {
          ...
          if (lspace) {
            ...
            if (rspace > 1)
              p +=
                  base_branch_cost * gpt[st + 1][en - 2][PT_U2] * Boltz(em.dangle5[stb][en1b][enb]);
          }
          if (rspace) {
            // |<   >5(   )<   >| 5' - Exterior loop
            // rspace > 0
            ...
            if (lspace > 1)
              p +=
                  base_branch_cost * gpt[st + 2][en - 1][PT_U2] * Boltz(em.dangle3[stb][st1b][enb]);
          }
          if (lspace && rspace) {
      ...
      for (int tpiv = st + 1; tpiv <= en + N; ++tpiv) {
        ...
        // baseAB indicates A bases left unpaired on the left, B bases left unpaired on the right.
        const BoltzEnergy base00 = gpt[st][piv][PT_P] * Boltz(pc.augubranch[stb][pb]);
        const BoltzEnergy base01 = gpt[st][pl][PT_P] * Boltz(pc.augubranch[stb][pl1b]);
        ...
        if (straddling) {
          ...
          // |  )  >>   <(   )<(  | Flush coax
          // straddling
          val = base00 * Boltz(em.stack[pb][prb][prb ^ 3][stb]) * gpt[pr][en][PT_U_WC];
          u += val;
          u2 += val;
          if (prb == G || prb == U) {
            val = base00 * Boltz(em.stack[pb][prb][prb ^ 1][stb]) * gpt[pr][en][PT_U_GU];
            u += val;
            u2 += val;
          }
          // |  ).  >>   <(   )<.(   | Right coax forward
        ...
        if (lspace) {
          const BoltzEnergy base10 = gpt[st + 1][piv][PT_P] * Boltz(pc.augubranch[st1b][pb]);
          const BoltzEnergy base11 = gpt[st + 1][pl][PT_P] * Boltz(pc.augubranch[st1b][pl1b]);
