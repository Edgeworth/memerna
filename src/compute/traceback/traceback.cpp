    ...
    if (en == -1) {
      ...
      for (en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
        ...
        if (a == EXT_RCOAX) {
          ...
          if (base11 + em.MismatchCoaxial(en1b, enb, stb, st1b) + gext[en + 1][EXT] ==
              gext[st][EXT_RCOAX]) {
        ...
        // (   )3<   > 3'
        if (base01 + em.dangle3[en1b][enb][stb] + gext[en + 1][EXT] == gext[st][EXT]) {
        ...
        // 5(   )<   > 5'
        if (base10 + em.dangle5[enb][stb][st1b] + gext[en + 1][EXT] == gext[st][EXT]) {
        ...
        // .(   ).<   > Terminal mismatch
        if (base11 + em.terminal[en1b][enb][stb][st1b] + gext[en + 1][EXT] == gext[st][EXT]) {
        ...
        if (en < N - 1) {
          ...
          // (   )(<   ) > Flush coax
          if (base01 + em.stack[en1b][enb][enb ^ 3][stb] + gext[en][EXT_WC] == gext[st][EXT]) {
          ...
          if ((enb == G || enb == U) &&
              base01 + em.stack[en1b][enb][enb ^ 1][stb] + gext[en][EXT_GU] == gext[st][EXT]) {
            gctd[st] = CTD_FCOAX_WITH_NEXT;
            gctd[en] = CTD_FCOAX_WITH_PREV;
            q.emplace(st, en - 1, DP_P);
      ...
      if (a == DP_P) {
        ...
        for (int ist = st + 1; ist < st + max_inter + 2; ++ist) {
          for (int ien = en - max_inter + ist - st - 2; ien < en; ++ien) {
            if (gdp[ist][ien][DP_P] < CAP_E) {
              const auto val = em.TwoLoop(r, st, en, ist, ien) + gdp[ist][ien][DP_P];
        ...
        // (3<   ><   >) 3'
        if (base_branch_cost + gdp[st + 2][en - 1][DP_U2] + em.dangle3[stb][st1b][enb] ==
            gdp[st][en][DP_P]) {
        ...
        // (<   ><   >5) 5'
        if (base_branch_cost + gdp[st + 1][en - 2][DP_U2] + em.dangle5[stb][en1b][enb] ==
            gdp[st][en][DP_P]) {
        ...
        // (.<   ><   >.) Terminal mismatch
        if (base_branch_cost + gdp[st + 2][en - 2][DP_U2] + em.terminal[stb][st1b][en1b][enb] ==
            gdp[st][en][DP_P]) {
        ...
        for (int piv = st + HAIRPIN_MIN_SZ + 2; piv < en - HAIRPIN_MIN_SZ - 2; ++piv) {
          ...
          if (base_branch_cost + gdp[st + 2][piv][DP_P] + em.multiloop_hack_b +
                  em.AuGuPenalty(st2b, plb) + gdp[piv + 1][en - 2][DP_U] + outer_coax ==
              gdp[st][en][DP_P]) {
          ...
          // (.   (   ).) Right outer coax
          if (base_branch_cost + gdp[st + 2][piv][DP_U] + em.multiloop_hack_b +
                  em.AuGuPenalty(prb, en2b) + gdp[piv + 1][en - 2][DP_P] + outer_coax ==
              gdp[st][en][DP_P]) {
          ...
          // (.(   ).   ) Left right coax
          if (base_branch_cost + gdp[st + 2][piv - 1][DP_P] + em.multiloop_hack_b +
                  em.AuGuPenalty(st2b, pl1b) + gdp[piv + 1][en - 1][DP_U] +
          ...
          // (   .(   ).) Right left coax
          if (base_branch_cost + gdp[st + 1][piv][DP_U] + em.multiloop_hack_b +
                  em.AuGuPenalty(pr1b, en2b) + gdp[piv + 2][en - 2][DP_P] +
          ...
          // ((   )   ) Left flush coax
          if (base_branch_cost + gdp[st + 1][piv][DP_P] + em.multiloop_hack_b +
                  em.AuGuPenalty(st1b, plb) + gdp[piv + 1][en - 1][DP_U] +
          ...
          // (   (   )) Right flush coax
          if (base_branch_cost + gdp[st + 1][piv][DP_U] + em.multiloop_hack_b +
                  em.AuGuPenalty(prb, en1b) + gdp[piv + 1][en - 1][DP_P] +
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        const auto base00 = gdp[st][piv][DP_P] + em.AuGuPenalty(stb, pb) + em.multiloop_hack_b;
        const auto base01 =
            gdp[st][piv - 1][DP_P] + em.AuGuPenalty(stb, pl1b) + em.multiloop_hack_b;
        const auto base10 = gdp[st + 1][piv][DP_P] + em.AuGuPenalty(st1b, pb) + em.multiloop_hack_b;
        const auto base11 =
            gdp[st + 1][piv - 1][DP_P] + em.AuGuPenalty(st1b, pl1b) + em.multiloop_hack_b;
        ...
        // (   )3<   > 3' - U, U2
        if (base01 + em.dangle3[pl1b][pb][stb] + right_unpaired == gdp[st][en][a]) {
          gctd[st] = CTD_3_DANGLE;
        ...
        // 5(   )<   > 5' - U, U2
        if (base10 + em.dangle5[pb][stb][st1b] + right_unpaired == gdp[st][en][a]) {
          gctd[st + 1] = CTD_5_DANGLE;
          q.emplace(st + 1, piv, DP_P);
          if (a == DP_U2 || right_unpaired) q.emplace(piv + 1, en, DP_U);
          goto loopend;
        ...
        if (base11 + em.terminal[pl1b][pb][stb][st1b] + right_unpaired == gdp[st][en][a]) {
          gctd[st + 1] = CTD_MISMATCH;
          q.emplace(st + 1, piv - 1, DP_P);
          if (a == DP_U2 || right_unpaired) q.emplace(piv + 1, en, DP_U);
          goto loopend;
        ...
        // (   )(<   ) > Flush coax - U, U2
        if (base01 + em.stack[pl1b][pb][pb ^ 3][stb] + gdp[piv][en][DP_U_WC] == gdp[st][en][a]) {
          gctd[st] = CTD_FCOAX_WITH_NEXT;
        ...
        if ((pb == G || pb == U) &&
            base01 + em.stack[pl1b][pb][pb ^ 1][stb] + gdp[piv][en][DP_U_GU] == gdp[st][en][a]) {
          gctd[st] = CTD_FCOAX_WITH_NEXT;
          gctd[piv] = CTD_FCOAX_WITH_PREV;
          q.emplace(st, piv - 1, DP_P);
