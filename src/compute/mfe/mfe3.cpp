  ...
  for (int st = N - 1; st >= 0; --st) {
    ...
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      if (ViableFoldingPair(r, st, en)) {
        ...
        for (int isz = 1; isz <= max_inter; ++isz) {
          mins[DP_P] = std::min(mins[DP_P],
              em.Bulge(r, st, en, st + 1 + isz, en - 1) + gdp[st + 1 + isz][en - 1][DP_P]);
          mins[DP_P] = std::min(mins[DP_P],
              em.Bulge(r, st, en, st + 1, en - 1 - isz) + gdp[st + 1][en - 1 - isz][DP_P]);
        }
        ...
        for (int isz = 4; isz <= max_inter; ++isz) {
          ...
          mins[DP_P] = std::min(mins[DP_P],
              val + em.InternalLoopAuGuPenalty(r[st + isz], en2b) + gdp[st + isz][en - 2][DP_P]);
          mins[DP_P] = std::min(mins[DP_P],
              val + em.InternalLoopAuGuPenalty(st2b, r[en - isz]) + gdp[st + 2][en - isz][DP_P]);
        }
        ...
        // here.
        mins[DP_P] = std::min(mins[DP_P],
            em.internal_1x1[stb][st1b][st2b][en2b][en1b][enb] + gdp[st + 2][en - 2][DP_P]);
        mins[DP_P] = std::min(mins[DP_P],
      ...
      for (auto cand : cand_st[CAND_U_RCOAX_FWD]) {
        const auto val = cand.energy - pc.min_mismatch_coax + gdp[cand.idx + 1][en][DP_U_RCOAX];
        mins[DP_U] = std::min(mins[DP_U], val);
        mins[DP_U2] = std::min(mins[DP_U2], val);
      }
