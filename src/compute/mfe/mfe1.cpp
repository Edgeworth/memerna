  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      if (ViableFoldingPair(r, st, en)) {
        ...
        // (3<   ><   >) 3'
        p_min = std::min(
            p_min, base_branch_cost + gdp[st + 2][en - 1][DP_U2] + em.dangle3[stb][st1b][enb]);
        // (<   ><   >5) 5'
        p_min = std::min(
            p_min, base_branch_cost + gdp[st + 1][en - 2][DP_U2] + em.dangle5[stb][en1b][enb]);
        // (.<   ><   >.) Terminal mismatch
        p_min = std::min(p_min,
        ...
        for (int piv = st + HAIRPIN_MIN_SZ + 2; piv < en - HAIRPIN_MIN_SZ - 2; ++piv) {
          ...
          p_min = std::min(p_min,
              base_branch_cost + gdp[st + 2][piv][DP_P] + pc.augubranch[st2b][plb] +
                  gdp[piv + 1][en - 2][DP_U] + outer_coax);
          // (.   (   ).) Right outer coax
          p_min = std::min(p_min,
              base_branch_cost + gdp[st + 2][piv][DP_U] + pc.augubranch[prb][en2b] +
                  gdp[piv + 1][en - 2][DP_P] + outer_coax);

          // (.(   ).   ) Left right coax
          p_min = std::min(p_min,
              base_branch_cost + gdp[st + 2][piv - 1][DP_P] + pc.augubranch[st2b][pl1b] +
                  gdp[piv + 1][en - 1][DP_U] + em.MismatchCoaxial(pl1b, plb, st1b, st2b));
          // (   .(   ).) Right left coax
          p_min = std::min(p_min,
              base_branch_cost + gdp[st + 1][piv][DP_U] + pc.augubranch[pr1b][en2b] +
                  gdp[piv + 2][en - 2][DP_P] + em.MismatchCoaxial(en2b, en1b, prb, pr1b));

          // ((   )   ) Left flush coax
          p_min = std::min(p_min,
              base_branch_cost + gdp[st + 1][piv][DP_P] + pc.augubranch[st1b][plb] +
                  gdp[piv + 1][en - 1][DP_U] + em.stack[stb][st1b][plb][enb]);
          // (   (   )) Right flush coax
          p_min = std::min(p_min,
              base_branch_cost + gdp[st + 1][piv][DP_U] + pc.augubranch[prb][en1b] +
                  gdp[piv + 1][en - 1][DP_P] + em.stack[stb][prb][en1b][enb]);
        }
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        const auto base00 = gdp[st][piv][DP_P] + pc.augubranch[stb][pb];
        const auto base01 = gdp[st][piv - 1][DP_P] + pc.augubranch[stb][pl1b];
        const auto base10 = gdp[st + 1][piv][DP_P] + pc.augubranch[st1b][pb];
        const auto base11 = gdp[st + 1][piv - 1][DP_P] + pc.augubranch[st1b][pl1b];
        ...
        u2_min = std::min(u2_min, base01 + em.dangle3[pl1b][pb][stb] + gdp[piv + 1][en][DP_U]);
        // 5(   )<   > 5' - U
        u_min = std::min(u_min, base10 + em.dangle5[pb][stb][st1b] + right_unpaired);
        u2_min = std::min(u2_min, base10 + em.dangle5[pb][stb][st1b] + gdp[piv + 1][en][DP_U]);
        // .(   ).<   > Terminal mismatch - U
        u_min = std::min(u_min, base11 + em.terminal[pl1b][pb][stb][st1b] + right_unpaired);
        u2_min =
            std::min(u2_min, base11 + em.terminal[pl1b][pb][stb][st1b] + gdp[piv + 1][en][DP_U]);
        // .(   ).<(   ) > Left coax - U
