  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      if (ViableFoldingPair(r, st, en)) {
        ...
        for (int ist = st + 1; ist < st + max_inter + 2; ++ist) {
          for (int ien = en - max_inter + ist - st - 2; ien < en; ++ien) {
            if (gdp[ist][ien][DP_P] < CAP_E)
              UPDATE_CACHE(DP_P, em.TwoLoop(r, st, en, ist, ien) + gdp[ist][ien][DP_P]);
          }
        }
        // Hairpin loops.
        ...
        // (3<   ><   >) 3'
        UPDATE_CACHE(
            DP_P, base_branch_cost + gdp[st + 2][en - 1][DP_U2] + em.dangle3[stb][st1b][enb]);
        // (<   ><   >5) 5'
        UPDATE_CACHE(
            DP_P, base_branch_cost + gdp[st + 1][en - 2][DP_U2] + em.dangle5[stb][en1b][enb]);
        // (.<   ><   >.) Terminal mismatch
        UPDATE_CACHE(DP_P,
        ...
        for (int piv = st + HAIRPIN_MIN_SZ + 2; piv < en - HAIRPIN_MIN_SZ - 2; ++piv) {
          ...
          UPDATE_CACHE(DP_P,
              base_branch_cost + gdp[st + 2][piv][DP_P] + em.multiloop_hack_b +
                  em.AuGuPenalty(st2b, plb) + gdp[piv + 1][en - 2][DP_U] + outer_coax);
          // (.   (   ).) Right outer coax
          UPDATE_CACHE(DP_P,
              base_branch_cost + gdp[st + 2][piv][DP_U] + em.multiloop_hack_b +
                  em.AuGuPenalty(prb, en2b) + gdp[piv + 1][en - 2][DP_P] + outer_coax);

          // (.(   ).   ) Left right coax
          UPDATE_CACHE(DP_P,
              base_branch_cost + gdp[st + 2][piv - 1][DP_P] + em.multiloop_hack_b +
                  em.AuGuPenalty(st2b, pl1b) + gdp[piv + 1][en - 1][DP_U] +
                  em.MismatchCoaxial(pl1b, plb, st1b, st2b));
          // (   .(   ).) Right left coax
          UPDATE_CACHE(DP_P,
              base_branch_cost + gdp[st + 1][piv][DP_U] + em.multiloop_hack_b +
                  em.AuGuPenalty(pr1b, en2b) + gdp[piv + 2][en - 2][DP_P] +
                  em.MismatchCoaxial(en2b, en1b, prb, pr1b));

          // ((   )   ) Left flush coax
          UPDATE_CACHE(DP_P,
              base_branch_cost + gdp[st + 1][piv][DP_P] + em.multiloop_hack_b +
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        UPDATE_CACHE(DP_U2, base01 + em.dangle3[pl1b][pb][stb] + gdp[piv + 1][en][DP_U]);
        // 5(   )<   > 5' - U
        UPDATE_CACHE(DP_U, base10 + em.dangle5[pb][stb][st1b] + right_unpaired);
        UPDATE_CACHE(DP_U2, base10 + em.dangle5[pb][stb][st1b] + gdp[piv + 1][en][DP_U]);
        // .(   ).<   > Terminal mismatch - U
        UPDATE_CACHE(DP_U, base11 + em.terminal[pl1b][pb][stb][st1b] + right_unpaired);
        UPDATE_CACHE(DP_U2, base11 + em.terminal[pl1b][pb][stb][st1b] + gdp[piv + 1][en][DP_U]);
        // .(   ).<(   ) > Left coax - U
        ...
        // (   )(<   ) > Flush coax - U
        val = base01 + em.stack[pl1b][pb][pb ^ 3][stb] + gdp[piv][en][DP_U_WC];
        UPDATE_CACHE(DP_U, val);
        UPDATE_CACHE(DP_U2, val);
        if (pb == G || pb == U) {
          val = base01 + em.stack[pl1b][pb][pb ^ 1][stb] + gdp[piv][en][DP_U_GU];
          UPDATE_CACHE(DP_U, val);
          UPDATE_CACHE(DP_U2, val);
        }
      }
